...
public class Drivetrain implements Subsystem {
    ...
    // This guarantees only one instance of Drivetrain should ever exist at once
    ...
    public static SimpleMotorFeedforward motorFeedForward = new SimpleMotorFeedforward(Constants.kS, Constants.kV,
            Constants.kA);
    ...
    // Declaring variables to store acceleration values from last loop (to calculate
    // acceleration)
    ...
    /**
     * Zeroes the last loop velocities used to calculate acceleration between closed
     * loop driving modes
     */
    ...
    // Returns the current measurement of the left drivetrain encoder in feet,
    // assuming 1024 encoder ticks per rotation and 4 inch diameter wheels
    public static double getLeftFeet() {
        return ((leftMotorA.getSelectedSensorPosition() / 1024.0) * 4 * Math.PI) / 12;
    }

    // Returns the current measurement of the right drivetrain encoder in feet,
    // assuming 1024 encoder ticks per rotation and 4 inch diameter wheels
    ...
    /**
     * Function used to set the talon speeds in closed loop. Called by
     * setClosedLoop(Double, Double) after calculation of kinematics feedforwards
     * and talon native speed units is complete
     * 
     * @param left    Velocity to set left side of drivetrain to in talon units per
     *                decisecond
     * @param leftFf  Feedforward for left side of drivetrain in Volts
     * @param right   Velocity to set right side of drivetrain to in talon units per
     *                decisecond
    ...
    // Static class to handle conversion from joystick inputs into left and right
    // side outputs
